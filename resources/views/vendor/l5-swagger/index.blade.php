    ...
    window.onload = function() {
        ...
        const ui = SwaggerUIBundle({
            ...
            responseInterceptor: (res) => {
                // on successful login response, store token + workspace
                if (
                res.url.match(/\/login$/)
                && res.body
                && res.body.success
                && res.body.data.token
                && res.body.data.workspace
                ) {
                const token = res.body.data.token;
                const ws    = res.body.data.workspace;
                localStorage.setItem('token', token);
                localStorage.setItem('workspace-key',  ws.id);
                localStorage.setItem('workspace-type', ws.type);

                // also update the UI “Authorize” button state
            ...
            requestInterceptor: (req) => {
                // read saved auth + workspace
                const token = localStorage.getItem('token');
                const wsKey = localStorage.getItem('workspace-key');
                const wsType = localStorage.getItem('workspace-type');
                console.log(token, wsKey, wsType)
                if (token) {
                req.headers['Authorization'] = 'Bearer ' + token;
                }
                if (wsKey && wsType) {
                req.headers['X-Workspace-Key']  = wsKey;
                req.headers['X-Workspace-Type'] = wsType;
                }
                return req;
            },
